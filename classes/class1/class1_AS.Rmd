...
knitr::opts_knit$set(root.dir = "~/Dánsko/uni/2nd semester/Methods II/resources/classes/class1")
...
```{r}
# Load data
hibbs <- read.table("~/Dánsko/uni/2nd semester/Methods II/resources/classes/data/ElectionsEconomy/data/hibbs.dat", header = TRUE)
...
Simulating data means using the properties of data extracted of a dataset and creating fictional datasets to further explore the relationships.
...
It follows a normal distribution, meaning that the data will be approximately equally distributed around the mean on both sides. When visualized, it will look resemble a Gaussian curve.
...
ggplot(data = data.frame(sim), aes(sim)) +
  geom_histogram()
```
...
```{r}
#sketch data: y = 30 + 10x
intercept <- 30
slope <- 10
error <- rnorm(20, 0, 3.9)
...
# with error
y_error <- y + error
df_error <- data.frame(x, y_error)

ggplot(data = df, aes(x = x, y = y_error)) +
  geom_point() +
  geom_abline(slope = 10, intercept = 30, color = "red", linetype = "solid") +
  theme_minimal()
...
plot(df_error$x, df_error$y)

model <- stan_glm(y_error ~ x, data = df_error)
...
```{r}
y_error2 <- y + (rnorm(20, 0, 10))

df_error2 <- data.frame(x, y_error2)

plot(df_error2$x, df_error2$y_error2)
model2 <- stan_glm(y_error2 ~ x, data=df_error2)
abline(coef(model2), col='red')
print(model2)

ggplot(data = df_error2, aes(x = x, y = y_error2)) +
  geom_point() +
  geom_abline(slope = 10, intercept = 30, color = "red", linetype = "solid") +
  theme_minimal()
```
...
```{r}
allnames_clean <- read_csv('~/Dánsko/uni/2nd semester/Methods II/resources/classes/data/Names/data/allnames_clean.csv')

lastletter_df <- allnames_clean %>%
  filter(sex == 'F') %>%
  mutate(last_letter = substr(name, nchar(name), nchar(name))) 

#reshape data
lastletter_long <- lastletter_df %>%
  pivot_longer(cols = starts_with("X"), names_to = "year", values_to = "presence") %>%
  filter(year != 'X')
...
lastletter_long %>%
  ...
  summarise(letter_count = sum(presence)) %>%
  ungroup() %>%
  group_by(year) %>%
  mutate(percentage = (letter_count / sum(letter_count)) * 100,
         last_letter = as.factor(last_letter)) %>%
  
  ggplot(aes(x = year, y = percentage, group = last_letter, color = last_letter)) +
  geom_line() +
  theme_minimal() +
  scale_x_discrete(breaks = c('X1900', 'X1950', 'X2000')) +
  labs(x = "Year", y = "Percentage of Girls' Names that Year", color = 'Last Letter') +
  ggtitle("Last Letter of Girls' Names Across Years") +
  geom_label_repel(aes(label = last_letter, size = NULL),
                             arrow = NULL,
                             nudge_y = 3,
                             segment.size  = 0.3)
