...
Hint A1* under Hints
```{r}
integer_sum <- function(){
  integer_list <- as.integer(c(1:100))
...
for (i in as.integer(c(1:100))) {
  integer_sum[i] <- integer_sum
}
```
...
integerlist <- numeric()
tracker <- sum(integerlist)
while (tracker < 1000) {
  integerlist[i] <- integerlist + as.integer(1)
...
n <- 10
while (n >= 0) {
  ifelse(n < 1, print('Lift off!'), print(n)) 
  n <- n - 1
}
